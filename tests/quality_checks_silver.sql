...
-- Null Check: CustomerID should never be NULL
-- Expectation: No Results
SELECT * FROM silver.crm_sales_details WHERE sls_cust_id IS NULL;

-- Null Check: ProductID should never be NULL
-- Expectation: No Results
SELECT * FROM silver.crm_sales_details WHERE sls_prd_key IS NULL;

-- Null Check: OrderDate should never be NULL
-- Expectation: No Results
...
-- Expectation: No Results
SELECT * FROM silver.crm_sales_details WHERE sls_order_dt > GETDATE();

-- Rejected Records Summary (Upgrade 3): Rows discarded during Bronze -> Silver
-- Expectation: Review counts per reason, investigate any unexpected spikes
SELECT source_table, reject_reason, COUNT(*) AS rejected_count
