...
Features:
    ...
    - NEW: Bad rows are captured (not discarded) into silver.rejected_records.
...
BEGIN
    ...
    DECLARE @rows_updated  INT;       -- NEW: Capture MERGE update counts
    DECLARE @rows_before   INT;       -- NEW: Row count before this table's load
    ...
    -- NEW: Running totals for the step-level execution log
    ...
    BEGIN TRY
        ...
        -- NEW (Upgrade 3): Capture rows with a NULL CustomerID that would otherwise be silently
        -- discarded by the "WHERE cst_id IS NOT NULL" filter below.
        ...
        BEGIN TRANSACTION;
            -- NEW: OUTPUT clause captures whether each merged row was an INSERT or an UPDATE
        ...
        EXEC audit.usp_log_table_statistics @batch_id, 'silver.crm_cust_info', @rows_before, @slv_cnt, @rows_inserted, @rows_updated, 0, DATEDIFF(SECOND, @start_time, @end_time);
        PRINT '>> -------------';
        ...
        BEGIN TRANSACTION;
            ...
            SET @rows_updated = @@ROWCOUNT; -- NEW: Rows expired = rows updated for this step
        ...
        IF @slv_cnt <> @brz_cnt
            INSERT INTO audit.data_quality_issues (batch_id, table_name, check_name, expected_value, actual_value, failed_rows, issue_description, check_layer)
            VALUES (@batch_id, 'silver.crm_prd_info', 'Duplicate Check', @brz_cnt, @slv_cnt, ABS(@brz_cnt - @slv_cnt), 'Duplicate active product keys detected', 'Silver');

        INSERT INTO audit.etl_log (batch_id, table_name, start_time, end_time, row_count, status)
        VALUES (@batch_id, 'silver.crm_prd_info', @start_time, @end_time, @rows_inserted, 'Success');
        EXEC audit.usp_log_table_statistics @batch_id, 'silver.crm_prd_info', @rows_before, (SELECT COUNT(*) FROM silver.crm_prd_info), @rows_inserted, @rows_updated, 0, DATEDIFF(SECOND, @start_time, @end_time);
        PRINT '>> -------------';
        ...
        -- NEW (Upgrade 3): Capture rows with an invalid OrderDate or a missing ProductID
        -- that would otherwise be silently loaded with a NULL order date.
        INSERT INTO silver.rejected_records (batch_id, source_table, record_data, reject_reason)
        SELECT
            @batch_id,
        ...
        IF ISNULL(@brz_sales_sum, 0) <> ISNULL(@slv_sales_sum, 0)
            INSERT INTO audit.data_quality_issues (batch_id, table_name, check_name, expected_value, actual_value, failed_rows, issue_description, check_layer)
            VALUES (@batch_id, 'silver.crm_sales_details', 'Revenue Check', @brz_sales_sum, @slv_sales_sum, 1, 'Sales amount mismatch during delta load', 'Silver');
        
        INSERT INTO audit.etl_log (batch_id, table_name, start_time, end_time, row_count, status)
        VALUES (@batch_id, 'silver.crm_sales_details', @start_time, @end_time, @rows_inserted, 'Success');
        EXEC audit.usp_log_table_statistics @batch_id, 'silver.crm_sales_details', @rows_before, (SELECT COUNT(*) FROM silver.crm_sales_details), @rows_inserted, 0, 0, DATEDIFF(SECOND, @start_time, @end_time);
        PRINT '>> -------------';
        ...
        BEGIN TRANSACTION;
            EXEC silver.load_metadata_driven @batch_id = @batch_id, @total_rejected = @metadata_rejected OUTPUT;
        COMMIT TRANSACTION;
