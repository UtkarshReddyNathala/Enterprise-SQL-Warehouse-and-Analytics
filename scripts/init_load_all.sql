...
BEGIN
    ...
    DECLARE @execution_id INT; -- NEW: Step-level execution id for the master run itself
    ...
    BEGIN TRY
        ...
        -- NEW: Open a step-level execution record for the master pipeline (Upgrade 1)
        ...
        -- NEW: Close the master step-level execution record as a success
    ...
    BEGIN CATCH
        ...
        -- NEW: Close the master step-level execution record as a failure
        EXEC audit.usp_execution_end
            @execution_id  = @execution_id,
            @status        = 'FAILED',
